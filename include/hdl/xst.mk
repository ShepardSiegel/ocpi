...
XstMakeIni=\
  ($(foreach l,$(Libraries) $(OcpiCores),echo $(notdir $(l))=$(call FindRelative,$(TargetDir),$(l)/$(call LibraryAccessTarget,$(Target)));)\
   $(foreach l,$(OcpiLibraries),echo $(notdir $(l))=$(strip $(call FindRelative,$(TargetDir),$(l)/lib/hdl/$(call LibraryAccessTarget,$(Target))/hdl));)\
  ) > $(XstIniFile);
XstMakeLso=\
...
XstOptions += -ifn $(XstPrjFile) -ofn $(Core) -top $(Core) -p $(Target) \
               -vlgincdir { $(foreach d,$(VerilogIncludeDirs),$(call FindRelative,$(TargetDir),$(d))) }
ifneq ($(OcpiLibraries)$(OcpiCores),)
XstOptions += -sd { $(foreach l,$(OcpiLibraries),$(call FindRelative,$(TargetDir),$(l)/lib/hdl/$(Target)))\
		    $(foreach c,$(OcpiCores),$(call FindRelative,$(TargetDir),$(c)/$(Target))) \
		  }
endif
Compile=\
  $(AT)echo Building $@ from top in $(Core)$(SourceSuffix)\; details in $(TargetDir)/xst.out.;\
  cd $(TargetDir);$(XstMakePrj)$(XstMakeLso)$(XstMakeIni)$(XstMakeScr)\
  ($(TIME) xst -ifn $(XstScrFile)) > xst.out;\
  grep -i error xst.out|grep -v '^WARNING:'; \
