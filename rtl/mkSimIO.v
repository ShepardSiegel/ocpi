...
//
// On Sat Sep 29 15:57:08 EDT 2012
//
//
// Ports:
// Name                         I/O  size props
...
module mkSimIO(CLK,
  ...
  begin
    ...
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_do_r_char && b__h960 != 32'hFFFFFFFF)
	$display("[%0d]: get_cp read 0x%x  Host->Simulator requests: readCount:%0d ",
		 v__h1319,
		 b__h960[7:0],
		 h2cpByteCount);
    if (RST_N != `BSV_RESET_VALUE)
    ...
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_do_w_char)
	$display("[%0d]: get_cp write 0x%x  Simulator->Host responses: writeCount:%0d ",
		 v__h1673,
		 respF$D_OUT,
		 cp2hByteCount);
  end
