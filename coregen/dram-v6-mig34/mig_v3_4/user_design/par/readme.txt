...
3. The design can be implemented in ISE Projnav GUI by invoking the
   "test.ise" project file.

4. In Linux operating systems, test.ise project can be invoked by executing
   the command 'ise test.ise' from the terminal.

Other files in PAR folder :

* "mig_v3_4.ucf" file is the constraint file for the design. This is used
...
* "icon5_cg.xco", "ila384_8_cg.xco", "vio_async_in256_cg.xco" and
  ...
  i.e. ila, vio and icon and set DEBUG_PORT parameter to "ON" in mig_v3_4
...
* "mig_v3_4.prj" file has commands to add all rtl files to the XST project
  for synthesis.

* "mig_v3_4.lso" file is a custom library search order file for XST synthesis.
