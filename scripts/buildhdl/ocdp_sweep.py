...
def build_xst_commands(module, w, hasPush, hasPull, d):
  """
  Build a Xilinx XST command file to control synthesis
  """
  print 'Variation: ' + 'Width:' + str(w) + '_Push' + str(hasPush) + '_Pull' + str(hasPull) + '_Debug' + str(d)
  ...
  f.write('-top ' + module + '\n')
  f.write('-generics {')
  f.write('WMI_S0_DATAPATH_WIDTH=' + str(w) + ', ')
  f.write('HAS_PUSH_LOGIC=' + str(hasPush) + ', ')
  f.write('HAS_PULL_LOGIC=' + str(hasPull) + ', ')
...
def bench_sweep():
  ...
  """
  for w in WidthList:                    # TODO Replace with list of sweep list
    for hasPush in HasPushList:
      for hasPull in HasPullList:
        for d in DebugList:
          build_xst_commands(module, w, hasPush, hasPull, d);
          suffixString =  'Width' + str(w) + '_Push' + str(hasPush) + '_Pull' + str(hasPull) + '_Debug' + str(d)
