...
def build_sweep_ocdp():
  ...
  S['moduleName']   = 'SMAdapter'           # Name of the module to be swept
  S['sweepList']    = ['Width', 'Debug']    # List of dimensions to sweep
  S['Width']        = [32, 64, 128, 256]
  S['WidthParam']   = ['WMI_M0_DATAPATH_WIDTH', 'WSI_S0_DATAPATH_WIDTH', 'WSI_M0_DATAPATH_WIDTH']
  S['Debug']        = [0, 1]
...
def build_xst_commands(S, T):
  ...
  f.write('-generics {')
  i = 0
  for s in S['sweepList']: # This iterates over all swept paramaters
    sp = s + 'Param'
    j = 0
    for p in S[sp]:  # This iterator covers when a single parameter value is applied to multiple paramater names
      f.write(p + '=' + str(S[s][T[i]])) # use the tuple T to index the param name and value fields
      if j+1 < len(S[sp]):
        f.write(', ')
      j += 1
    if i+1 < len(S['sweepList']):
...
def build_rodin_commands(S, T):
  """
  Build a Xilinx Rodin command file to control synthesis
  """
  f = open('xst_tmp.xst', 'w')
  f.write('run\n')
  f.write('-ifn ' + S['moduleName'] + '.prj\n')
