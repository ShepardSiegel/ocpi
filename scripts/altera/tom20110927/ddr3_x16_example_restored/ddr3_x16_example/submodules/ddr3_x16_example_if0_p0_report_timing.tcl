...
##iming analysis for the read capture, write, PHY
...
##hat has all the details of the timing analysis, and this
##file only serves as the top-level timing analysis flow.
##
##To reduce data lookups in all the procuedures that perform
##he individual timing analysis, data that is needed for
##multiple procedures is lookup up in this file and passed
##o the various parameters.  These data include both values
##hat are applicable over all operating conditions, and those
##hat are applicable to only one operating condition.
##
##In addition to the data that is looked up, the script
##and the underlying procedures use various other data
##hat are stored in TCL sets and include the following:
##
##(.)     : Holds the memory timing parameters
...
source "$script_dir/ddr3_x16_example_if0_p0_report_timing_core.tcl"
...
foreach inst $instances {
	...
	if {[llength $instances] <= 1} {
		set fbasename "${::GLOBAL_ddr3_x16_example_if0_p0_corename}"
	} else {
		set fbasename "${::GLOBAL_ddr3_x16_example_if0_p0_corename}_${inst_id}"
	}
	...
	foreach opc $opcs {
		...
		if {!$in_gui && [array exists assumptions_cache] &&  [info exists assumptions_cache(${::GLOBAL_ddr3_x16_example_if0_p0_corename}-$inst)] } {
			set assumptions_valid $assumptions_cache(${::GLOBAL_ddr3_x16_example_if0_p0_corename}-$inst)
			...
			set assumptions_cache(${::GLOBAL_ddr3_x16_example_if0_p0_corename}-$inst) $assumptions_valid
		}	
		...
		perform_resync_timing_analysis $opcs $opcname $inst ${::GLOBAL_ddr3_x16_example_if0_p0_corename} $family $DQS_min_max $io_std $interface_type $period pins t summary MP IP board fpga SSN
	...
	set summary [lsort -command ddr3_x16_example_if0_p0_sort_proc $summary]
	...
	if { [get_operating_conditions_number] == 0 } {
		...
		puts $f "Core: ${::GLOBAL_ddr3_x16_example_if0_p0_corename} - Instance: $inst"
		puts $f "Path, Setup Margin, Hold Margin"
	} else {
		set f [open $fname a]
	}

	post_message -type info "Core: ${::GLOBAL_ddr3_x16_example_if0_p0_corename} - Instance: $inst"
